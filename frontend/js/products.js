...
const DEMO_PRODUCTS = [
  {
    ...
    category: "Fashion",
    description:
      "Premium handcrafted Ankara kaftan tailored in Lagos. Lightweight and breathable.",
    images: [
      "https://images.unsplash.com/photo-1539109136881-3be0616acf4b?w=800",
    ],
    seller: { name: "Lagos Threads", phone: "2348012345678" },
  },
  {
    id: "d2",
    ...
    description:
      "Crystal-clear sound with 24-hour battery life and noise cancellation.",
    images: [
      "https://images.unsplash.com/photo-1606220588913-b3aacb4d2f46?w=800",
    ],
    seller: { name: "AbujaTech", phone: "2348087654321" },
  },
  {
    id: "d3",
    ...
    category: "Beauty",
    description:
      "100% organic Nigerian shea butter set with cleanser, moisturizer and lip balm.",
    images: ["https://images.unsplash.com/photo-1556228720-195a672e8a03?w=800"],
    seller: { name: "PureGlow NG", phone: "2348023456789" },
  },
  {
    id: "d4",
    ...
    category: "Fashion",
    description:
      "Genuine leather, handmade in Aba. Fits a 10-inch tablet and essentials.",
    images: [
      "https://images.unsplash.com/photo-1591561954557-26941169b49e?w=800",
    ],
    seller: { name: "Aba Leatherworks", phone: "2348034567890" },
  },
  {
    id: "d5",
    ...
    stock: 120,
    category: "Electronics",
    description: "Wi-Fi controlled RGB bulb. Works with Alexa and Google Home.",
    images: [
  ...
  },
  {
    ...
    category: "Food",
    description:
      "Authentic Nigerian jollof spice blend. Family-recipe, ready in minutes.",
    images: [
      "https://images.unsplash.com/photo-1604329760661-e71dc83f8f26?w=800",
    ],
    seller: { name: "Mama's Kitchen", phone: "2348045678901" },
  },
  {
    id: "d7",
    name: "Adire Throw Pillow Pair",
    price: 7500,
    stock: 22,
    category: "Home",
    description: "Hand-dyed adire pillow covers. Set of two, 18x18 inches.",
    images: [
      "https://images.unsplash.com/photo-1567016432779-094069958ea5?w=800",
    ],
    seller: { name: "Yoruba Living", phone: "2348056789012" },
  },
  {
    id: "d8",
    ...
    category: "Sports",
    description:
      "Set of 5 latex-free resistance bands with door anchor and carry bag.",
    images: [
      "https://images.unsplash.com/photo-1517836357463-d25dfeac3438?w=800",
    ],
    seller: { name: "FitNaija", phone: "2348067890123" },
  },
];
...
const Products = {
  ...
  async fetchAll() {
    try {
      ...
      Products.cache = list.length
        ? list.map(Products.normalize)
        : DEMO_PRODUCTS;
    } catch (e) {
  ...
  async fetchAllForSeller() {
    try {
      const data = await API.listProductsForSeller();
      const list = Array.isArray(data)
        ? data
        : data?.products || data?.data || [];
      Products.cache = list.length
    ...
    return `
      <article class="card card-hover overflow-hidden flex flex-col">
        <a href="product.html?id=${encodeURIComponent(
          p.id
        )}" class="block aspect-square overflow-hidden bg-slate-100">
          <img src="${img}" alt="${
      p.name
        ...
        <div class="p-4 flex-1 flex flex-col">
          <div class="flex items-start justify-between gap-2">
            <h3 class="font-semibold text-slate-900 clamp-2 leading-snug">
              <a href="product.html?id=${encodeURIComponent(
                p.id
              )}" class="hover:text-emerald-700">${p.name}</a>
            </h3>
            <span class="badge ${
              inStock ? "badge-success" : "badge-danger"
            } shrink-0">${inStock ? "In stock" : "Sold out"}</span>
          </div>
          <p class="text-xs text-slate-500 mt-1">${p.category}</p>
          <div class="mt-3 flex items-end justify-between">
            <span class="font-display text-xl font-bold">${UI.formatNaira(
              p.price
            )}</span>
          </div>
          <a href="${UI.whatsappUrl(
            p.seller?.phone,
            p.name
          )}" target="_blank" rel="noopener" class="btn-wa mt-4 w-full justify-center">
